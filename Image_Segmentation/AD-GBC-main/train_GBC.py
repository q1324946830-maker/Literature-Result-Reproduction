...
def main():
    ...
    if config['optimizer'] == 'Adam':
        # dup_ids = {id(p) for p in others}.intersection({id(p) for p in gbc_centers_params})
        # [(name, id(param)) for name, param in model.named_parameters() if id(param) in dup_ids]
        # print(dup_ids)
